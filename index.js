...
if (answers.projectType === 'React - TypeScript') {
  fs.writeFileSync('.eslintrc.json', JSON.stringify(reactTsEslintConfig, null, 2));

  await install(
    'eslint prettier eslint-config-prettier eslint-plugin-prettier typescript @typescript-eslint/eslint-plugin @typescript-eslint/parser eslint-plugin-react eslint-plugin-react-hooks eslint-plugin-react-perf',
  );
}

if (answers.projectType === 'React - JavaScript') {
  fs.writeFileSync('.eslintrc.json', JSON.stringify(reactJsEslintConfig, null, 2));
  await install(
    'eslint prettier eslint-config-prettier eslint-plugin-prettier eslint-plugin-react eslint-plugin-react-hooks eslint-plugin-react-perf',
  );
}
